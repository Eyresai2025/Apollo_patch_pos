...
Fixes:
- correct import from pip patchify lib
- returns patches output folder
- no top-level run unless __main__
"""

import glob, cv2, os, os.path as osp
from patchify import patchify   # ✅ pip package import
import numpy as np
        ...
        if not cover_edges:
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    if not cv2.imwrite(out_path, patch_img):
