...
- Runner/converter saves only: height_8bit PNG, height_16bit PNG, one PLY
"""
...
class LaserCaptureTab(QWidget):
    ...
    def build_ui(self) -> None:
        ...
        production_note = QLabel(
            "Current production output: one full-resolution ASCII PLY + height_8bit PNG + "
            "height_16bit PNG only. Metadata and RAW are temporary and deleted unless checked."
        )
        ...
        table_hint = QLabel(
            "Only rows marked Enabled are passed to the runner. Second laser is disabled by "
            "default until M0006994 is connected and tested."
        )
        ...
        self.laser_table.setColumnCount(13)
    ...
    def _set_default_table_widths(self) -> None:
        widths = [105, 70, 250, 105, 90, 105, 115, 95, 90, 80, 105, 100, 100]
    ...
    def build_laser_configs(self) -> Dict[str, Dict[str, object]]:
        ...
        for row in range(self.laser_table.rowCount()):
            ...
            profile_rate = self._to_optional_float(self._cell_text(row, 10))
            exposure = self._to_optional_float(self._cell_text(row, 11))
            write_locked = self._cell_text(row, 12) == "1"
    ...
    def _configuration_lines(self, configs: Dict[str, Dict[str, object]]) -> list[str]:
        ...
        lines = [
            ...
            (
                ...
                "save=height_8bit,height_16bit,ply_only"
            ),
        ]
