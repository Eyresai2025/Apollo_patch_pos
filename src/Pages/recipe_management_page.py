...
class RecipeManagementPage(QWidget):
    ...
    def load_selected_recipe_to_machine(self):
        ...
        reply = QMessageBox.question(
            ...
            (
                ...
                "- write Tyre Name directly to DB75 byte 290 as PLC Recipe Name STRING and verify\n"
    ...
    def _format_plc_result_message(self, result: Dict[str, Any]) -> str:
        ...
        msg = (
            ...
            f"Recipe Name STRING (DB75 byte 290): {status(name_result.get('verified', not name_result.get('enabled', False)))}\n"
            f"Recipe Name PLC Max Length: {name_result.get('effective_max_len', '-')}\n"
            ...
            f"Activation Transition: {('ALREADY ACTIVE BEFORE LOAD' if result.get('active_recipe_already_selected_before') else ('YES - CHANGED TO SELECTED RECIPE' if result.get('activation_transition_observed') else 'NO TRANSITION OBSERVED'))}\n"
