...
class OffsetCalculationPage(QWidget):
    ...
    DEFAULT_TARGET_MATCH_THRESHOLD = 0.70
    ...
    def _build_ui(self) -> None:
        ...
        info = QLabel(
            "Use calibration images containing two visible markers in the selected target view."
        )
        ...
        header.addStretch(1)
        badge = QLabel("SIDEWALL + TARGET MARKER PIPELINE")
        ...
        self._add_path_row(1, "sidewall_input", "Calibration Sidewall Image Folder", "folder")
        self._add_path_row(2, "target_input", "Calibration Target Image Folder", "folder")
        self._add_path_row(3, "sidewall_template", "Sidewall R Template", "image")
        self._add_path_row(4, "target_template", "Target Marker Template", "image")
        self._add_path_row(5, "output_json", "Output Calibration JSON", "save_json")
    ...
    def refresh_context(self) -> None:
        for role, state in self.states.items():
            ...
            if sidewall_template and (
                not state.get("sidewall_template")
                or not Path(str(state.get("sidewall_template"))).is_file()
            ):
    ...
    def _validate_active_config(self) -> Optional[dict]:
        ...
        sidewall_template = Path(str(state.get("sidewall_template") or ""))
        target_template = Path(str(state.get("target_template") or ""))
        output_text = str(state.get("output_json") or "").strip()

        if not sidewall_input.exists():
            QMessageBox.warning(
                self, "Offset Calculation", "Choose a valid sidewall calibration input."
            )
            return None
        if not target_input.exists():
            ...
            return None
        if not sidewall_template.is_file():
            QMessageBox.warning(
                self, "Offset Calculation", "Choose a valid sidewall R template."
            )
            return None
        ...
        return {
            ...
            "sidewall_r_template": sidewall_template,
            ...
            "r_match_threshold": self.DEFAULT_R_MATCH_THRESHOLD,
    ...
    def start_active_calculation(self) -> None:
        ...
        reply = QMessageBox.question(
            ...
            f"Run the sidewall-to-{display} offset calculation?\n\n"
