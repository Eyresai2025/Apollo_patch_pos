...
except Exception:  # pragma: no cover - threshold scripts can run standalone
    get_config = None
...
class PatchCoreScorer:
    ...
    def _nearest_memory_distance(self, query_patches: torch.Tensor) -> torch.Tensor:
        ...
        chunk_size = _runtime_int("PATCHCORE_MEMORY_BANK_CHUNK_SIZE", MEMORY_BANK_CHUNK_SIZE)
        for start in range(0, self.memory_bank.shape[0], chunk_size):
            bank_chunk = self.memory_bank[start : start + chunk_size]
