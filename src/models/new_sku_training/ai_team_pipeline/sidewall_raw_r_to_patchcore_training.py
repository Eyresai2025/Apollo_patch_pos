...
# Optional fast R detection modules used by the optimized inference pipeline.
# Keep detect_and_crop_fast.py and r_locator_fast.py beside this training file
# when R_DETECTION_METHOD = "fast". The code falls back to the tiled detector
# if fast detection is disabled or fails and R_FAST_FALLBACK_TO_TILED is True.
try:
    import detect_and_crop_fast as dcf
    import r_locator_fast as rlf
except Exception:  # modules may not be present in older training folders
    dcf = None
    rlf = None
...
RAW_TRAIN_FOLDER = Path(
    r"C:\Users\eyres\Downloads\sidewall1 (1)"
)
...
PREPROCESS_OUTPUT_ROOT = Path(
    r"C:\Users\eyres\Downloads\sidewall1 (1)\prepared_training"
)

# Final PatchCore memory-bank model.
OUT_PATH = Path(
    r"C:\Users\eyres\Downloads\sidewall1 (1)\sw2_amazer4g_model.pth"
)

# Complete timing log.
TIMING_CSV = Path(
    r"C:\Users\eyres\Downloads\sidewall1 (1)\training_timings.csv"
)

# Raw-preparation status report.
PREPROCESS_REPORT_JSON = Path(
    r"C:\Users\eyres\Downloads\sidewall1 (1)\preprocess_report.json"
)
...
# R-detection method:
#   "tiled" -> original training detector in this file.
#   "fast"  -> taught/recipe based R locator used by the optimized inference pipeline.
#              Requires detect_and_crop_fast.py, r_locator_fast.py, and a valid recipe.
#
# Keep fallback enabled during validation/training so one difficult image does not
# stop the full training run. The status JSON records whether fast or fallback was used.
...
# Do not train/preprocess on recipe verification, ROI/template or debug images
# accidentally placed inside the raw training folder.
    ...
    if rlf is None or dcf is None:
        if R_FAST_FALLBACK_TO_TILED:
            print(
                ...
                "Training will use tiled R detection fallback."
            )
            return None

        raise ImportError(
            "R_DETECTION_METHOD='fast' requires detect_and_crop_fast.py "
            "and r_locator_fast.py beside this training script."
        )
    ...
    if not R_RECIPE_PATH.is_file():
        if R_FAST_FALLBACK_TO_TILED:
            print(
                ...
                f"Training will use tiled R detection fallback: {R_RECIPE_PATH}"
            )
            return None
...
def detect_r_bands_for_training(
    raw_image: np.ndarray,
    template_blurred: np.ndarray,
    fast_recipe=None,
    ...
    to the original tiled detector. Crop pixels are still taken from the
    ...
    if method == "fast" and fast_recipe is not None and dcf is not None:
        try:
            match_boxes, r_bands, metadata = dcf.detect_r_bands_fast(
                raw_image,
                fast_recipe,
            )
            ...
            if len(r_bands) >= 2:
                ...
                metadata.setdefault("method", "fast")
        ...
        match_boxes, r_bands, metadata = detect_r_bands(
            raw_image,
            template_blurred,
        )

        metadata = dict(metadata or {})
        metadata["method"] = "tiled_fallback"
        metadata["fallback_used"] = True
        metadata["fast_failure_reason"] = fast_failure_reason
        return match_boxes, r_bands, metadata
    ...
    metadata = dict(metadata or {})
    metadata.setdefault("method", "tiled")
    metadata.setdefault("fallback_used", False)
    return match_boxes, r_bands, metadata
...
def prepare_one_raw_image(
    ...
    status = {
        ...
        "R_detection_method": detection_metadata.get(
            "method",
...
def main() -> None:
    ...
    print("=" * 80)
    print(
        "RAW GOOD TYRES -> FAST/TILED R CROP -> Vit_patch.py "
        "-> PATCHCORE TRAINING"
    )
    print("=" * 80)
    ...
    print(
        f"[{now_s()}] R detection method: {normalize_r_detection_method()}"
        + (
            f" | recipe={R_RECIPE_PATH}"
            if fast_recipe is not None
            else " | using tiled/fallback path"
        )
    )
    ...
    preprocessing_report = {
        ...
        "R_detection_method_requested": normalize_r_detection_method(),
        "R_recipe_path": (
            str(R_RECIPE_PATH)
