...
def detect_r_bands(
    ...
    row_gap: int,
    blur_kernel: tuple[int, int] = (5, 5),
    search_x_start_ratio: float = 0.0,
    search_x_end_ratio: float = 1.0,
    ...
    if not (
        0.0 <= search_x_start_ratio
        < search_x_end_ratio
        <= 1.0
    ):
        raise ValueError(
            "Invalid R search X ratios: "
            f"{search_x_start_ratio}, "
            f"{search_x_end_ratio}"
        )

    search_x_start = int(
        round(
            image_width
            * search_x_start_ratio
        )
    )
    ...
    if (
        ...
        raise ValueError(
            "Configured R search band is narrower "
            "than the R template."
        )
    ...
    metadata = {
        ...
        "search_x_start_ratio": float(
            search_x_start_ratio
        ),
        "search_x_end_ratio": float(
            search_x_end_ratio
        ),
        "search_x_start": int(
            search_x_start
        ),
        "search_x_end_exclusive": int(
            search_x_end
        ),
        "search_width": int(
            search_x_end
