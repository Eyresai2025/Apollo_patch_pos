...
SIDE_NAME = "innerwall"
SIDE_LABEL_PREFIX = "INNER"
ENABLE_YOLO_DIMENSIONS = True
...
def align_crop_from_preprocessed(
    ...
    # New values from cycle_engine.py / Maincycle
    ...
    """
    New flow for innerwall:

        polarized innerwall image
        -> crop using current sidewall R1/R2 anchor
        -> apply saved innerwall offset_ratio
        -> resize to RESIZE_CROP_TO
        -> X edge alignment using artifacts
        -> return final crop

    No R detection is done inside innerwall.
    """

    if sidewall_r_anchor is None:
        raise RuntimeError(
            f"[{SIDE_NAME}] sidewall_r_anchor is required. "
            "innerwall must use current sidewall R1/R2 from cycle_engine.py."
        )
    ...
    if save_template_path is not None:
        cv2.imwrite(save_template_path, crop_bgr)

        meta_path = os.path.splitext(save_template_path)[0] + "_offset_xalign_meta.json"
...
def build_calibration_pipeline(
    ...
    x_ref_path = os.path.join(x_align_artifacts_dir, f"{SIDE_NAME}_x_ref_edges.json")
    if os.path.exists(x_ref_path):
        os.remove(x_ref_path)
        print(f"[{SIDE_NAME.upper()} XALIGN] Removed old X reference: {x_ref_path}")
...
def load_calibration_artifacts_from_dir(
    ...
    if reference_band_info is None:
        if reference_r is None or len(reference_r) < 2:
            ...
            print("[ARTIFACT][WARN] reference_band_info.pt and valid reference_r.pt missing; continuing for non-R live flow")
        else:
            ...
            if y2 > y1:
                ...
                reference_band_info = None
                print("[ARTIFACT][WARN] invalid reference_r band; continuing for non-R live flow")
...
def load_runtime(
    ...
    """
    Runtime loader for innerwall Live inference.

    Important:
    - Innerwall is a non-R side.
    - R detection is NOT done inside this file.
    - cycle_engine.py must pass sidewall_r_anchor, offset_ratio, and x_align_artifacts_dir.
    """
    ...
    # Innerwall does not need R detector.
    # Keep override only for compatibility with cycle_engine.py.
    r_detector = None

    if r_detector_override is not None:
        r_detector = r_detector_override
        print("[RUNTIME] shared R-detector received but not used inside innerwall")
