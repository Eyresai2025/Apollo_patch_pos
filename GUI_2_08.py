...
# ---------------------------------------------------------------------------
# GLOBAL APOLLO DIALOG + TOOLTIP THEME
# ---------------------------------------------------------------------------
# Standard Qt dialogs inherit the operating-system palette unless explicitly
# styled.  On machines using a dark Windows palette this previously produced
# black QMessageBox/QInputDialog bodies with unreadable dark text.
APOLLO_GLOBAL_DIALOG_STYLE = r"""
    QToolTip {
...
def _normalise_button_caption(text: str) -> str:
    """Return a clean caption suitable for an operator tooltip."""
...
def _default_button_tooltip(button) -> str:
    """Generate a useful fallback tooltip for buttons created across pages."""
    caption = _normalise_button_caption(getattr(button, "text", lambda: "")())
    key = caption.lower()
    exact = {
        "refresh": "Refresh the current page with the latest available information.",
        "apply": "Apply the selected filters and refresh the displayed records.",
        "clear": "Clear the current filters or fields.",
        "close": "Close this page and return to the previous screen.",
        "cancel": "Close this dialog without applying changes.",
        "ok": "Confirm this action and close the dialog.",
        "previous": "Show the previous page of records.",
        "next": "Show the next page of records.",
        "browse": "Browse and select a file or folder.",
        "open folder": "Open the configured output folder in File Explorer.",
        "back to live": "Return to the Live inspection dashboard.",
        "stop inspection": "Stop the Live inspection safely after the active cycle boundary.",
        "auto start": "Send the configured PLC Auto Start pulse.",
        "servo reset": "Send the configured PLC all-servo reset pulse.",
    }
    if key in exact:
        return exact[key]
    if not caption:
    ...
    return f"Select '{caption}' to perform this action."
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, session: SessionContext):
        ...
        # Setup UI
        self.setup_ui()
        # Give every current and dynamically-created button an operator-friendly
        # tooltip while preserving any explicit page-specific tooltip.
        self._apply_default_button_tooltips(self)
        QTimer.singleShot(0, lambda: self._apply_default_button_tooltips(self))
    ...
    def _ensure_button_tooltip(self, button) -> None:
        """Apply a fallback tooltip only when the page did not define one."""
        try:
            if button is None or str(button.toolTip() or "").strip():
                return
            button.setToolTip(_default_button_tooltip(button))
        except Exception:
            pass

    def _apply_default_button_tooltips(self, root=None) -> None:
        """Apply consistent tooltips to all visible application buttons."""
        root = root or self
        try:
            for button in root.findChildren(QPushButton):
                self._ensure_button_tooltip(button)
    ...
    def setup_header_bar(self, parent_layout):
        ...
        self.back_btn.setToolTip("Return to the Live inspection dashboard.")
        ...
        self.stop_inspection_btn.setToolTip(
            "Stop Live inspection safely. An active tyre cycle is allowed to finish, "
            "and the next PLC trigger will not be accepted."
        )
        ...
        self.auto_start_btn.setToolTip(
            "Send the configured PLC Auto Start pulse after verifying the machine is safe."
        )
        ...
        self.servo_reset_btn.setToolTip(
            "Send the configured PLC all-servo reset pulse. Use only in a safe machine state."
        )
    ...
    def setup_sidebar(self, main_layout):
        ...
        for text, icon_name, slot, permission in buttons:
            ...
            sidebar_tooltips = {
                "System Monitor": "Open hardware readiness, Lab Camera AI and the Alarm Center.",
                "Live": "Select the active SKU and start or resume Live tyre inspection.",
                "Device": "Configure and verify connected camera devices.",
                "Capture": "Open Camera and Laser capture configuration tools.",
                "Axis Status": "View PLC axis, servo and motion status.",
                "Run New SKU": "Start the guided New SKU capture and preparation workflow.",
                "Recipe Management": "Create, review and manage PLC-to-SKU recipe mappings.",
                "Repeatability": "Run repeatability captures and comparison checks.",
