...
def main() -> int:
    ...
    if failed:
        ...
        # Query failures are shared by several checks, so print the root cause
        # even though QUERY_ERROR is not itself a check key.
        if "QUERY_ERROR" in details:
            print(f"  QUERY_ERROR: {details['QUERY_ERROR']}")
        return 1
    print("Status                           : PASSED")
    return 0
