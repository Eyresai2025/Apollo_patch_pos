...
class _PyMongoLikeDatabase:
    """Mimics PyMongo's explicit rejection of truth-value testing."""
...
class InspectionHistoryV4Tests(unittest.TestCase):
    ...
    def test_constructor_accepts_real_pymongo_style_database(self):
        collection = _Collection([])
        database = _PyMongoLikeDatabase(collection)
        service = InspectionHistoryService(database=database)
        self.assertIs(service.database, database)
        self.assertIs(service.collection, collection)
